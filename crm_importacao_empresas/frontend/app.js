...
const adminUsuariosPage = document.querySelector('#admin-usuarios-page');
const adminUserCreatePage = document.querySelector('#admin-user-create-page');
const adminUserSettingsPage = document.querySelector('#admin-user-settings-page');
const adminUsersRefreshButton = document.querySelector('#admin-users-refresh');
const adminUsersUpdatedAt = document.querySelector('#admin-users-updated-at');
const adminTotalUsers = document.querySelector('#admin-total-users');
const adminOnlineUsers = document.querySelector('#admin-online-users');
const adminActiveSessions = document.querySelector('#admin-active-sessions');
const adminTotalFeatures = document.querySelector('#admin-total-features');
const adminUsersTable = document.querySelector('#admin-users-table');
const adminActiveAccessList = document.querySelector('#admin-active-access-list');
const adminUserCreateForm = document.querySelector('#admin-user-create-form');
const adminCreatePermissions = document.querySelector('#admin-create-permissions');
const adminUserCreateStatus = document.querySelector('#admin-user-create-status');
const adminUserSettingsForm = document.querySelector('#admin-user-settings-form');
const adminSettingsUserSelect = document.querySelector('#admin-settings-user-select');
const adminUserSettingsFields = document.querySelector('#admin-user-settings-fields');
const adminSettingsPermissions = document.querySelector('#admin-settings-permissions');
const adminUserSettingsStatus = document.querySelector('#admin-user-settings-status');
...
let currentCrmFeatures = [];
let adminUsersCache = [];
let adminSettingsCurrentUser = null;
...
function getCrmSessionId() {
    const storageKey = 'leadbenefits.crm.sessionId';
    let sessionId = localStorage.getItem(storageKey);

    if (!sessionId) {
        sessionId = `sessao-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
...
function getStoredUserId() {
    return Number(localStorage.getItem('leadbenefits.crm.userId') || 0);
}

function setStoredUserId(idUsuario) {
    if (idUsuario) {
        localStorage.setItem('leadbenefits.crm.userId', String(idUsuario));
    }
}

function clearStoredUserId() {
    localStorage.removeItem('leadbenefits.crm.userId');
}

function canUseFeature(code) {
    if (!code) {
        return true;
    }

    if (currentCrmUser?.perfil === 'ADMIN') {
        return true;
    }

    return (currentCrmUser?.permissoes || []).some((permission) => (
        permission.codigo === code && permission.permitido
    ));
}
...
function applyFrontendPermissions() {
    const permissions = {
        ...
        '#empresas/cadastrar': 'cadastrar-nova-empresa',
        '#empresas/importacao': 'importacao-empresas',
        '#admin/usuarios': 'administracao-usuarios',
        '#admin/usuarios/novo': 'configuracoes-usuario',
        '#admin/usuarios/configuracoes': 'configuracoes-usuario'
    };

    for (const [href, feature] of Object.entries(permissions)) {
        setElementVisibility(`a[href="${href}"]`, canUseFeature(feature));
    }

    setElementVisibility('#manual-company-open', canUseFeature('cadastrar-nova-empresa'));
    setElementVisibility('#save-prospection-button', canUseFeature('registrar-historico-contato'));
    setElementVisibility('#dashboard-atrasados-all', canUseFeature('agenda-contatos'));
    document.querySelectorAll('.company-flag-button').forEach((button) => {
        button.hidden = !canUseFeature('marcar-empresa-prioritaria');
    });
}
...
function showLogin(message = '') {
    ...
    if (loginStatus) {
        loginStatus.textContent = message || 'Informe seus dados para acessar.';
    }
...
function showApp() {
    ...
    if (currentUserName) {
        currentUserName.textContent = currentCrmUser?.nome || '-';
    }
...
async function registerAccessSession() {
    try {
        const response = await fetch('/api/admin/acesso', {
            method: 'POST'
        });
        const payload = await response.json();

        if (!response.ok) {
            throw new Error(payload.error || 'Erro ao registrar acesso.');
        }

        currentCrmUser = payload.usuario || null;
        currentCrmFeatures = payload.funcionalidades || [];
        setStoredUserId(currentCrmUser?.idUsuario);
    } catch (error) {
        currentCrmUser = null;
        currentCrmFeatures = [];
...
function getPermissionCodesFrom(container) {
    if (!container) {
        return [];
    }
...
function groupFeatures(features = []) {
    return features.reduce((groups, feature) => {
        const category = feature.categoria || 'Geral';
...
function renderPermissionGrid(container, features = [], selectedCodes = []) {
    if (!container) {
        return;
    }
    ...
    if (!features.length) {
        container.innerHTML = '<p class="dashboard-empty">Nenhuma funcionalidade cadastrada.</p>';
        return;
    }
        ...
        <fieldset class="permission-group">
            ...
            ${items.map((feature) => `
                <label class="permission-option">
                    <input type="checkbox" value="${escapeHtml(feature.codigo)}" ${selected.has(feature.codigo) ? 'checked' : ''}>
                    <span>
                        <strong>${escapeHtml(feature.nome)}</strong>
                        <small>${escapeHtml(feature.codigo)}</small>
                        <em>${escapeHtml(feature.descricao || '')}</em>
                    </span>
                </label>
            `).join('')}
        </fieldset>
...
function getDefaultPermissionCodesForProfile(profile, features = currentCrmFeatures) {
    const allCodes = features.map((feature) => feature.codigo);
    ...
    if (profile === 'GESTOR') {
        return allCodes.filter((codigo) => codigo !== 'configuracoes-usuario');
    }
...
async function loadAdminData() {
    const response = await fetch('/api/admin/resumo');
    const payload = await response.json();

    if (!response.ok) {
        throw new Error(payload.error || 'Erro ao carregar administracao.');
    }
...
function renderAdminUsersTable(users = []) {
    ...
    if (!users.length) {
        adminUsersTable.innerHTML = '<tr><td colspan="7">Nenhum usuario cadastrado.</td></tr>';
        return;
    }

    adminUsersTable.innerHTML = users.map((user) => `
        <tr>
            <td><strong>${escapeHtml(user.nome || '-')}</strong><br><span>${escapeHtml(user.cargo || '-')}</span></td>
            <td>${escapeHtml(user.email || '-')}</td>
            <td>${escapeHtml(user.perfil || '-')}</td>
            <td><span class="status-pill ${user.status === 'ATIVO' ? 'is-active' : 'is-inactive'}">${escapeHtml(user.status || '-')}</span></td>
            <td>${formatAdminNumber(user.permissoesAtivas)}</td>
            <td>${formatDateTime(user.ultimoAcessoEm)}</td>
            <td><a class="link-button" href="#admin/usuarios/configuracoes?id=${encodeURIComponent(user.idUsuario)}">Editar</a></td>
        </tr>
    `).join('');
}

function renderActiveAccessList(accesses = []) {
    if (!adminActiveAccessList) {
        return;
    }

    if (!accesses.length) {
        adminActiveAccessList.innerHTML = '<p class="dashboard-empty">Nenhum acesso ativo nos ultimos 5 minutos.</p>';
        return;
    }

    adminActiveAccessList.innerHTML = accesses.map((access) => `
        <article class="active-access-row">
            <div>
                <strong>${escapeHtml(access.nomeUsuario || 'Visitante')}</strong>
                <span>${escapeHtml(access.email || access.ip || '-')}</span>
            </div>
            <time>${formatDateTime(access.ultimoAcessoEm)}</time>
        </article>
    `).join('');
}

function renderAdminDashboard(data) {
    adminUsersCache = data?.usuarios || [];

    if (adminTotalUsers) {
        adminTotalUsers.textContent = formatAdminNumber(data?.metricas?.usuariosCadastrados);
    }

    if (adminOnlineUsers) {
        adminOnlineUsers.textContent = formatAdminNumber(data?.metricas?.usuariosOnline);
    }

    if (adminActiveSessions) {
        adminActiveSessions.textContent = formatAdminNumber(data?.metricas?.sessoesAtivas);
    }

    if (adminTotalFeatures) {
        adminTotalFeatures.textContent = formatAdminNumber(data?.metricas?.funcionalidades);
    }

    if (adminUsersUpdatedAt) {
        adminUsersUpdatedAt.textContent = `Atualizado em ${formatDateTime(data?.geradoEm)}`;
    }

    renderAdminUsersTable(adminUsersCache);
    renderActiveAccessList(data?.acessosRecentes || []);
}

async function loadAdminDashboard() {
    if (!adminUsuariosPage) {
        return;
    }

    if (adminUsersUpdatedAt) {
        adminUsersUpdatedAt.textContent = 'Atualizando administracao...';
    }

    if (adminUsersRefreshButton) {
        adminUsersRefreshButton.disabled = true;
    }

    try {
        await registerAccessSession();
        renderAdminDashboard(await loadAdminData());
    } catch (error) {
        if (adminUsersUpdatedAt) {
            adminUsersUpdatedAt.textContent = error.message;
        }
    } finally {
        if (adminUsersRefreshButton) {
...
async function loadAdminFeatures() {
    if (currentCrmFeatures.length) {
        return currentCrmFeatures;
    }
...
async function setupCreateUserPage() {
    const features = await loadAdminFeatures();
    const selected = getDefaultPermissionCodesForProfile(adminUserCreateForm?.elements.perfil?.value || 'OPERADOR', features);

    renderPermissionGrid(adminCreatePermissions, features, selected);
}
...
async function loadAdminUsersForSettings(selectedId = 0) {
    ...
    adminUsersCache = payload.usuarios || [];
    currentCrmFeatures = payload.funcionalidades || currentCrmFeatures;

    if (adminSettingsUserSelect) {
        adminSettingsUserSelect.innerHTML = [
            '<option value="">Selecione</option>',
            ...adminUsersCache.map((user) => `
                <option value="${escapeHtml(user.idUsuario)}" ${Number(selectedId) === Number(user.idUsuario) ? 'selected' : ''}>
...
async function loadAdminUserSettings(idUsuario) {
    if (!idUsuario) {
        adminSettingsCurrentUser = null;
        if (adminUserSettingsFields) {
            adminUserSettingsFields.hidden = true;
        }
        if (adminUserSettingsStatus) {
            adminUserSettingsStatus.textContent = 'Selecione um usuario para ajustar as configuracoes.';
        }
        return;
    }

    const response = await fetch(`/api/admin/usuarios?id=${encodeURIComponent(idUsuario)}`);
    const payload = await response.json();
    ...
    adminSettingsCurrentUser = user;
    currentCrmFeatures = payload.funcionalidades || currentCrmFeatures;
    setFormValues(adminUserSettingsForm, {
        nome: user.nome,
        email: user.email,
        cargo: user.cargo,
    ...
    renderPermissionGrid(
        ...
        user.permissoes || [],
        (user.permissoes || []).filter((item) => item.permitido).map((item) => item.codigo)
    );

    if (adminUserSettingsFields) {
        adminUserSettingsFields.hidden = false;
    }

    if (adminUserSettingsStatus) {
        adminUserSettingsStatus.textContent = 'Ajuste as permissoes e salve as alteracoes.';
    }
}

async function setupUserSettingsPage() {
    const params = new URLSearchParams((window.location.hash.split('?')[1] || ''));
    const selectedId = Number(params.get('id') || 0);

    try {
        await loadAdminUsersForSettings(selectedId);
        await loadAdminUserSettings(selectedId);
    } catch (error) {
        if (adminUserSettingsStatus) {
            adminUserSettingsStatus.textContent = error.message;
        }
    }
}

function buildAdminUserPayload(form, permissionsContainer) {
    ...
    return {
        ...
        perfil: String(formData.get('perfil') || 'OPERADOR').trim(),
        status: String(formData.get('status') || 'ATIVO').trim(),
        senhaTemporaria: String(formData.get('senhaTemporaria') || '').trim(),
        observacoes: String(formData.get('observacoes') || '').trim(),
        permissoes: getPermissionCodesFrom(permissionsContainer)
    };
}
...
async function saveAdminUser(event) {
    ...
    if (!adminUserCreateForm || !adminUserCreateStatus) {
        return;
    }

    const submitButton = adminUserCreateForm.querySelector('button[type="submit"]');
    adminUserCreateStatus.textContent = 'Salvando usuario...';
    ...
    try {
        const response = await fetch('/api/admin/usuarios', {
            ...
            body: JSON.stringify(buildAdminUserPayload(adminUserCreateForm, adminCreatePermissions))
        });
        const payload = await response.json();
        ...
        await setupCreateUserPage();
        adminUserCreateStatus.textContent = `Usuario ${payload.usuario?.nome || ''} cadastrado.`;
    } catch (error) {
        adminUserCreateStatus.textContent = error.message;
    } finally {
        if (submitButton) {
            submitButton.disabled = false;
        }
...
async function saveAdminUserSettings(event) {
    event.preventDefault();

    if (!adminUserSettingsForm || !adminSettingsCurrentUser) {
        return;
    }

    const submitButton = adminUserSettingsForm.querySelector('button[type="submit"]');

    if (adminUserSettingsStatus) {
        adminUserSettingsStatus.textContent = 'Salvando alteracoes...';
    }
    ...
    try {
        const response = await fetch(`/api/admin/usuarios?id=${encodeURIComponent(adminSettingsCurrentUser.idUsuario)}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify(buildAdminUserPayload(adminUserSettingsForm, adminSettingsPermissions))
        });
        const payload = await response.json();
        ...
        adminSettingsCurrentUser = payload.usuario;
        await loadAdminUsersForSettings(adminSettingsCurrentUser.idUsuario);
        if (adminUserSettingsStatus) {
            adminUserSettingsStatus.textContent = 'Configuracoes salvas.';
        }
    } catch (error) {
        if (adminUserSettingsStatus) {
            adminUserSettingsStatus.textContent = error.message;
...
function setMainView(view) {
    ...
    const showAdminUsuarios = view === 'admin-usuarios';
    const showAdminCreate = view === 'admin-usuario-novo';
    const showAdminSettings = view === 'admin-usuario-configuracoes';
    const showModule = showImportacao || showCadastro || showAdminUsuarios || showAdminCreate || showAdminSettings;

    mainCrmViews.forEach((element) => {
        element.hidden = showModule;
    });
    ...
    if (adminUsuariosPage) {
        adminUsuariosPage.hidden = !showAdminUsuarios;
    }

    if (adminUserCreatePage) {
        adminUserCreatePage.hidden = !showAdminCreate;
    }

    if (adminUserSettingsPage) {
        adminUserSettingsPage.hidden = !showAdminSettings;
    }
...
function handleNavigation() {
    ...
    if (route === '#empresas/importacao') {
        setMainView('importacao');
        return;
    }

    if (route === '#empresas/cadastrar') {
        setMainView('cadastrar');
        return;
    }

    if (route === '#admin/usuarios') {
        setMainView('admin-usuarios');
        return;
    }

    if (route === '#admin/usuarios/novo') {
        setMainView('admin-usuario-novo');
        return;
    }

    if (route === '#admin/usuarios/configuracoes') {
        setMainView('admin-usuario-configuracoes');
        return;
    }
...
function renderFlagButton(item) {
    if (!canUseFeature('marcar-empresa-prioritaria')) {
        return '';
    }
...
adminUsersRefreshButton?.addEventListener('click', loadAdminDashboard);
adminUserCreateForm?.addEventListener('submit', saveAdminUser);
adminUserSettingsForm?.addEventListener('submit', saveAdminUserSettings);
adminSettingsUserSelect?.addEventListener('change', () => {
    loadAdminUserSettings(Number(adminSettingsUserSelect.value || 0)).catch((error) => {
        if (adminUserSettingsStatus) {
            adminUserSettingsStatus.textContent = error.message;
        }
    });
});
adminUserCreateForm?.elements.perfil?.addEventListener('change', () => {
    renderPermissionGrid(
        adminCreatePermissions,
        currentCrmFeatures,
        getDefaultPermissionCodesForProfile(adminUserCreateForm.elements.perfil.value)
    );
});
adminUserSettingsForm?.elements.perfil?.addEventListener('change', () => {
    renderPermissionGrid(
        adminSettingsPermissions,
        currentCrmFeatures,
        getDefaultPermissionCodesForProfile(adminUserSettingsForm.elements.perfil.value)
    );
});
